...
const TopRated = () => {
    ...
    useEffect(() => {
        axios(`https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=${language}&page=${currentPage}`)
            .then(({data}) => setTopRated(data.results))
    }, [currentPage, language])

    return (
        <div className=" mx-auto  bg-neutral-800  p-5">
            <div className="md:container mx-auto">
                <h1 className="text-5xl text-center my-5 text-white font-medium">Top rated films</h1>
                <div className="flex flex-row flex-wrap font-serif text-white font-serif">
                    {
                        topRated.map(el => (
